...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			//  1 获取beanFactory 父类GenericApplicationContext的BeanFactory:DefaultListableBeanFactory
			//  2  直接返回 不用创建
			// 对于 ClassPathXmlApplicationContex
			 // 1 调用  org.springframework.context.support.AbstractRefreshableApplicationContext.refreshBeanFactory
			//  2 如果存在beanFactory 先销毁bean 注册到关闭beanFactory
			// 再创建beanFactory
			//3 注册传入spring的 xml 配置文件中的bean 注册到beanFactory
			// 4 将beanFactory 幅值给容器，返回beanFactory
			ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory();

			// Prepare the bean factory for use in this context.
			//准备bean工厂：指定beanFactory的类加载器，添加后置处理器，并注册到缺省环境bean
			//beanFactory 添加了2个后置处理器
			//ApplicationContextAwareProcessor  ApplicationListenerDetector
			prepareBeanFactory(beanFactory);

			try {
				// Allows post-processing of the bean factory in context subclasses.
				//beanFactory 加载其他后置处理器。比如ServletContextAwareProcessor等
	...
	/**
	 *
	 * 此处实现BeanFacotry接口的方法，
	 * 实际上 其子类 具有BeanFactory 对象属性。 在初始化时候，定义好了{@link GenericApplicationContext}
